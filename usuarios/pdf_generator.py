...
def gerar_pdf_listagem_usuarios(request):
    ...
    # Lista de Usuários (excluindo superusuários)
    usuarios = request.user.__class__.objects.filter(is_superuser=False, deleted_at__isnull=True).order_by('nome_completo')
    ...
    for status_code, status_display in request.user.Status.choices:
    ...
    for perfil_code, perfil_display in request.user.Perfil.choices:
...
def gerar_pdf_usuarios_por_perfil(perfil, request):
    ...
    # Obter o nome do perfil para exibição
    perfil_display = dict(request.user.Perfil.choices).get(perfil, perfil)
    ...
    # Estatísticas por status dentro do perfil
    story.append(Paragraph("ESTATÍSTICAS POR STATUS", styles['Subtitulo']))
    for status_code, status_display in request.user.Status.choices:
        count = usuarios.filter(status=status_code).count()
        if count > 0:
    ...
    if usuarios.exists():
        ...
        for i, user in enumerate(usuarios, 1):
            ...
            # Serviços periciais (especialmente importante para peritos)
            servicos = user.servicos_periciais.all()
            if servicos:
