...
def gerar_pdf_usuario(usuario, request):
    ...
    if usuario.updated_at:
        add_linha(
            "Última Atualização", usuario.updated_at.strftime("%d/%m/%Y às %H:%M:%S")
        )
    ...
    if hasattr(usuario, "deleted_at") and usuario.deleted_at:
        add_linha(
            "Data de Exclusão", usuario.deleted_at.strftime("%d/%m/%Y às %H:%M:%S")
        )
...
def gerar_pdf_listagem_usuarios(request):
    ...
    usuarios = User.objects.filter(
        is_superuser=False, deleted_at__isnull=True
    ...
    if usuarios.exists():
        ...
        for i, user in enumerate(usuarios, 1):
            story.append(
                Paragraph(f"<b>{i}. {user.nome_completo}</b>", styles["Subtitulo"])
            )

            story.append(
                Paragraph(f"<b>Email:</b> {user.email}", styles["UsuarioItem"])
            )
            story.append(Paragraph(f"<b>CPF:</b> {user.cpf}", styles["UsuarioItem"]))
            story.append(
                Paragraph(
                    f"<b>Status:</b> {user.get_status_display()}", styles["UsuarioItem"]
                )
            )
            if user.perfil:
                story.append(
                    Paragraph(
                        f"<b>Perfil:</b> {user.get_perfil_display()}",
                        styles["UsuarioItem"],
                    )
                )
            if user.telefone_celular:
                story.append(
                    Paragraph(
                        f"<b>Telefone:</b> {user.telefone_celular}",
                        styles["UsuarioItem"],
                    )
                )
            ...
            story.append(
                Paragraph(
                    f"<b>Cadastrado em:</b> {data_cadastro}", styles["UsuarioItem"]
                )
            )
            ...
            if servicos:
                ...
                story.append(
                    Paragraph(
                        f"<b>Serviços:</b> {', '.join(servicos_nomes)}",
                        styles["UsuarioItem"],
                    )
                )
...
def gerar_pdf_usuarios_por_perfil(perfil, request):
    ...
    usuarios = User.objects.filter(
        perfil=perfil, is_superuser=False, deleted_at__isnull=True
    ).order_by("nome_completo")
    ...
    if usuarios.exists():
        ...
        story.append(
            Paragraph(
                f"Nenhum usuário encontrado para o perfil {perfil_display}.",
