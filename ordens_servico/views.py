...
class OrdemServicoViewSet(viewsets.ModelViewSet):
    ...
    queryset = (
        OrdemServico.all_objects.select_related(
            "ocorrencia",
    ...
    def create(self, request, *args, **kwargs):
        ...
        if (
            hasattr(Ocorrencia, "Status")
            and ocorrencia.status == Ocorrencia.Status.FINALIZADA
        ):
            return Response(
                {
        ...
        serializer = self.get_serializer(
            data=request.data, context={"request": request, "ocorrencia": ocorrencia}
        )
        ...
        response_serializer = OrdemServicoSerializer(
            ordem_servico, context={"request": request}
        )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if (
            instance.ocorrencia
            and instance.ocorrencia.perito_atribuido
            and request.user.is_authenticated
            and request.user.id == instance.ocorrencia.perito_atribuido.id
            and hasattr(instance, "registrar_visualizacao")
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if hasattr(instance, "soft_delete"):
            ...
            return Response(
                {"error": "Operação de exclusão não suportada."},
    ...
    def pendentes_ciencia(self, request):
        ...
        return Response({"count": ordens.count(), "ordens": dados})
    ...
    def tomar_ciencia(self, request, pk=None, **kwargs):
        ...
        if (
            not ordem_servico.ocorrencia
            or request.user != ordem_servico.ocorrencia.perito_atribuido
        ):
            return Response(
        ...
        if ordem_servico.ciente_por:
            data_ciencia = (
                ordem_servico.data_ciencia.strftime("%d/%m/%Y às %H:%M")
    ...
    def reiterar(self, request, pk=None, **kwargs):
            ...
            if ordenada_por_id_val:
                try:
                    ...
                    raise ValidationError(
                        "Usuário 'ordenada por' selecionado não existe."
                    )
        ...
        response_serializer = OrdemServicoSerializer(
            nova_os, context={"request": request}
        )
    ...
    def justificar_atraso(self, request, pk=None, **kwargs):
        ...
        if not getattr(ordem_servico, "esta_vencida", False):
            prazo_str = (
                ordem_servico.data_prazo.strftime("%d/%m/%Y")
                if ordem_servico.data_prazo
                else "Não definido"
            )
        ...
        serializer = self.get_serializer(
            data=request.data
        )  # Usa JustificarAtrasoSerializer
        serializer.is_valid(raise_exception=True)
    ...
    def concluir(self, request, pk=None, **kwargs):
        ...
        if ordem_servico.status == OrdemServico.Status.AGUARDANDO_CIENCIA:
            ...
            if ordem_servico.ocorrencia and ordem_servico.ocorrencia.perito_atribuido:
                perito_nome = (
                    ordem_servico.ocorrencia.perito_atribuido.nome_completo
                    or perito_nome
                )
            return Response(
                {
    ...
    def lixeira(self, request, **kwargs):
        ...
        queryset = self.filter_queryset(
            self.get_queryset()
        )  # get_queryset deve retornar os deletados aqui
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = self.get_serializer(
                page, many=True
            )  # Usa OrdemServicoLixeiraSerializer
    ...
    def restaurar(self, request, **kwargs):
        ...
        try:
            ...
            if hasattr(OrdemServico, "all_objects"):
                ...
                instance = OrdemServico.objects.get(
                    **filter_kwargs
                )  # Pode falhar se o manager padrão exclui
        except OrdemServico.DoesNotExist:
            return Response(
                {"error": "OS não encontrada."}, status=status.HTTP_404_NOT_FOUND
            )

        # Assume deleted_at existe
        if not getattr(instance, "deleted_at", None):
            return Response(
                {"message": "Esta OS não está na lixeira."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if hasattr(instance, "restore"):
            try:
                ...
                return Response(
                    {"error": f"Erro interno ao restaurar: {e}"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
                )
            ...
            return Response(
                {"error": "Operação de restauração não suportada."},
                status=status.HTTP_405_METHOD_NOT_ALLOWED,
            )
    ...
    def gerar_pdf(self, request, pk=None):
        ...
        is_perito_dest = (
            ordem.ocorrencia
            and ordem.ocorrencia.perito_atribuido
            and request.user.id == ordem.ocorrencia.perito_atribuido.id
        )

        # Assume ocultar_detalhes_ate_ciencia existe
        if (
            is_perito_dest
            and hasattr(ordem, "ocultar_detalhes_ate_ciencia")
            and ordem.ocultar_detalhes_ate_ciencia()
        ):
            return Response(
    ...
    def gerar_pdf_oficial(self, request, *args, **kwargs):
        ...
        is_perito_dest = (
            ordem_servico.ocorrencia
            and ordem_servico.ocorrencia.perito_atribuido
            and request.user.id == ordem_servico.ocorrencia.perito_atribuido.id
        )

        if (
            is_perito_dest
            and hasattr(ordem_servico, "ocultar_detalhes_ate_ciencia")
            and ordem_servico.ocultar_detalhes_ate_ciencia()
        ):
            return Response(
                {
    ...
    def relatorios_gerenciais(self, request):
        ...
        resumo_geral = queryset_filtrado.aggregate(
            ...
            vencidas=Count(
                "id",
                filter=Q(data_prazo__isnull=False, data_prazo__lt=data_atual)
                & ~Q(status="CONCLUIDA"),
            ),
        ...
        producao_por_perito = (
            queryset_filtrado.values("ocorrencia__perito_atribuido__nome_completo")
            .annotate(
                perito_id=F("ocorrencia__perito_atribuido_id"),
                total_emitidas=Count("id"),
                concluidas=Count("id", filter=Q(status="CONCLUIDA")),
                em_andamento=Count("id", filter=Q(status="EM_ANDAMENTO")),
                vencidas=Count(
        ...
        for p_data in producao_por_perito:
            conc = p_data["concluidas"]
            c_np = p_data["cumpridas_no_prazo"]
            producao_detalhada.append(
                {
                    "perito_id": p_data["perito_id"],
                    "perito": p_data["ocorrencia__perito_atribuido__nome_completo"]
                    or "Sem perito",
                    **p_data,
                    "taxa_cumprimento_prazo": round(
                        (c_np / conc * 100) if conc > 0 else 0, 1
                    ),
                }
            )
        ...
        por_unidade = (
            queryset_filtrado.values("unidade_demandante__nome")
            .annotate(
                unidade_id=F("unidade_demandante_id"),
                total=Count("id"),
                concluidas=Count("id", filter=Q(status="CONCLUIDA")),
                em_andamento=Count("id", filter=Q(status="EM_ANDAMENTO")),
                vencidas=Count(
        ...
        reiteracoes_stats = queryset_filtrado.aggregate(
            total_originais=Count("id", filter=Q(numero_reiteracao=0)),
            total_reiteracoes=Count("id", filter=Q(numero_reiteracao__gt=0)),
            primeira_reiteracao=Count("id", filter=Q(numero_reiteracao=1)),
            segunda_reiteracao=Count("id", filter=Q(numero_reiteracao=2)),
            terceira_ou_mais=Count("id", filter=Q(numero_reiteracao__gte=3)),
            total_emitidas=Count("id"),
        )
        ...
        cnp_geral = taxa_aggr["cumpridas_no_prazo"]
        cca_geral = taxa_aggr["cumpridas_com_atraso"]
        taxa_cumprimento = {
            "total_concluidas": tc_geral,
            "cumpridas_no_prazo": cnp_geral,
            "cumpridas_com_atraso": cca_geral,
            "percentual_no_prazo": round(
                (cnp_geral / tc_geral * 100) if tc_geral > 0 else 0, 1
            ),
            "percentual_com_atraso": round(
                (cca_geral / tc_geral * 100) if tc_geral > 0 else 0, 1
            ),
        }
        ...
        prazos_aggr = (
            ...
            .aggregate(
                media_duracao=Avg("duracao"), media_prazo_concedido=Avg("prazo_dias")
            )
        )
        dias_medio = (
            prazos_aggr.get("media_duracao").days
            if prazos_aggr.get("media_duracao")
            else 0
        )
        prazos_stats = {
            "tempo_medio_conclusao_dias": dias_medio,
            "prazo_medio_concedido": round(
                prazos_aggr.get("media_prazo_concedido") or 0, 1
            ),
        }
    ...
    def relatorios_gerenciais_pdf(self, request):
        ...
        if response.status_code != 200:
            return Response(
                {"error": "Dados indisponíveis para PDF."}, status=response.status_code
            )
        dados = response.data
        filtros_aplicados = {}
        if request.query_params.get("data_inicio"):
            filtros_aplicados["Data Início"] = request.query_params.get("data_inicio")
        if request.query_params.get("data_fim"):
            filtros_aplicados["Data Fim"] = request.query_params.get("data_fim")
        if request.query_params.get("status"):
            filtros_aplicados["Status"] = request.query_params.get("status")
        perito_id_param = request.query_params.get("perito_id")
        if perito_id_param:
            try:
                perito = User.objects.get(pk=perito_id_param)
                filtros_aplicados["Perito"] = perito.nome_completo
            except User.DoesNotExist:
                filtros_aplicados["Perito"] = f"ID {perito_id_param}"
    ...
    def estatisticas_os(self, request):
        ...
        filterset_fields = (
            self.filterset_class.get_fields() if self.filterset_class else {}
        )
        if (
            servico_id
            and "ocorrencia__servico_pericial" not in filterset_fields
            and "servico_id" not in filterset_fields
        ):  # Ajuste chave se necessário
            try:
                queryset = queryset.filter(
                    ocorrencia__servico_pericial_id=int(servico_id)
                )
            except (ValueError, TypeError):
        ...
        stats = queryset.aggregate(
            total=Count("id"),
            aguardando_ciencia=Count(
                "id", filter=Q(status=OrdemServico.Status.AGUARDANDO_CIENCIA)
            ),
            ...
            vencidas=Count(
                "id",
                filter=Q(data_prazo__isnull=False)
