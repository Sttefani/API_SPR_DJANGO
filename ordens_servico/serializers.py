...
class OrdemServicoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'data_ciencia', 'ip_ciencia', 'created_at', 'updated_at',
            'numero_reiteracao',
            # Relações
            'ocorrencia', 'created_by', 'updated_by', 'ordenada_por', 'ciente_por',
            'unidade_demandante', 'autoridade_demandante', 'procedimento',
            'tipo_documento_referencia', 'os_original',
            # Campos calculados
    ...
    def get_acao_necessaria(self, obj):
        ...
        if (not obj.ciente_por and 
            perito_destinatario and 
            user.id == perito_destinatario.id):
            return "TOMAR_CIENCIA"
        
        # Se está aberta mas não iniciou o trabalho
        if (obj.status == OrdemServico.Status.ABERTA and
            perito_destinatario and
            user.id == perito_destinatario.id):
            return "INICIAR_TRABALHO"
        
        # Se está vencida e sem justificativa
        if (obj.esta_vencida and 
            not obj.justificativa_atraso and
            perito_destinatario and
            user.id == perito_destinatario.id):
        ...
        if (obj.esta_vencida and 
            obj.dias_restantes and
            obj.dias_restantes < -3 and
    ...
    def get_reiteracoes(self, obj):
        ...
        if obj.numero_reiteracao == 0:  # É a original
            reiteracoes = obj.reiteracoes.filter(deleted_at__isnull=True).order_by('numero_reiteracao')
            return [{
                'id': r.id,
    ...
    def to_representation(self, instance):
        ...
        if (not instance.ciente_por and 
            perito_destinatario and 
            user.id == perito_destinatario.id and
            ...
            ret['mensagem'] = "Dê ciência nesta OS para visualizar os detalhes completos."
        else:
...
class OrdemServicoLixeiraSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrdemServico
        fields = [
            'id', 'numero_os', 'prazo_dias', 'status',
            'ocorrencia', 'deleted_at', 'deleted_by'
        ]