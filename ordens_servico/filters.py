...
class OrdemServicoFilter(django_filters.FilterSet):
    ...
    status = django_filters.ChoiceFilter(
        choices=OrdemServico.Status.choices, label="Status"
    )
    ...
    data_fim = django_filters.DateFilter(
        field_name="created_at", lookup_expr="date__lte", label="Emitida até"
    )

    # ===== FILTROS BOOLEANOS =====
    vencida = django_filters.BooleanFilter(
        method="filter_vencida", label="Apenas Vencidas"
    )

    sem_ciencia = django_filters.BooleanFilter(
        method="filter_sem_ciencia", label="Sem Ciência"  # Método correto já existia
    )
    ...
    def filter_urgencia(self, queryset, name, value):
        ...
        queryset_ativas = queryset.filter(data_prazo__isnull=False).exclude(
        ...
        if value == "vermelho":
            ...
            return queryset_ativas.filter(
                data_prazo__gte=amanha, data_prazo__lte=depois_de_amanha
            )
            ...
            return queryset_ativas.filter(
                data_prazo__gte=dia_mais_3, data_prazo__lte=dia_mais_4
            )
    ...
    def filter_sem_ciencia(self, queryset, name, value):
        ...
        q_sem_ciencia = Q(status=OrdemServico.Status.AGUARDANDO_CIENCIA) & Q(
            ciente_por__isnull=True
        )
        if value:
    ...
    def filter_com_justificativa(self, queryset, name, value):
        ...
        q_com_justificativa = Q(justificativa_atraso__isnull=False) & ~Q(
            justificativa_atraso__exact=""
        )
        if value:
