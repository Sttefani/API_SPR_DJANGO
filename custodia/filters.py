...
class VestigioFilter(django_filters.FilterSet):
    ...
    # "Vestígios comigo" — guarda física atual do usuário autenticado (visão CRM
    # do perito): recebidos por ele (user_destino), SEM transferência pendente de
    # saída e ainda não finalizados. É o material parado na bancada aguardando
    # exame, para depois ser movimentado de volta à custódia ou finalizado.
    comigo = django_filters.BooleanFilter(method='filtrar_comigo')

    def filtrar_comigo(self, queryset, name, value):
        if not value:
            return queryset
        ...
        return (
            queryset
            .filter(user_destino=user)
            .exclude(status=Vestigio.Status.FINALIZADO)
            .exclude(movimentacoes_custodia__aceito=False)  # nada em trânsito
            .distinct()
        )
