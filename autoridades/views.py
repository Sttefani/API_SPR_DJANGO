...
class AutoridadeViewSet(viewsets.ModelViewSet):
    ...
    def lixeira(self, request):
        lixeira_qs = Autoridade.all_objects.select_related('cargo').filter(deleted_at__isnull=False).order_by('-deleted_at')
    ...
    def restaurar(self, request, pk=None):
        ...
        if instance.deleted_at is None:
            return Response({'detail': 'Esta autoridade não está deletada.'}, status=status.HTTP_400_BAD_REQUEST)
        instance.restore()
