...
class RelatoriosGerenciaisViewSet(viewsets.ViewSet):
    ...
    def get_queryset(self):
        ...
        if not (user.is_superuser or user.perfil == 'ADMINISTRATIVO'):
            queryset = queryset.filter(servico_pericial__in=user.servicos_periciais.all())
    ...
    def list(self, request):
        ...
        por_servico = servicos_queryset.annotate(
            total=Coalesce(Count('ocorrencias', filter=Q(ocorrencias__in=queryset)), 0),
            finalizadas=Coalesce(Count('ocorrencias', filter=Q(ocorrencias__in=queryset, ocorrencias__status='FINALIZADA')), 0),
            em_analise=Coalesce(Count('ocorrencias', filter=Q(ocorrencias__in=queryset, ocorrencias__status='EM_ANALISE')), 0)
