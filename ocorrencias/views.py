...
class OcorrenciaViewSet(viewsets.ModelViewSet):
    ...
    def relatorios_gerenciais(self, request):
        ...
        por_grupo_principal = queryset.annotate(
            ...
            grupo_codigo=Case(
                When(classificacao__parent__isnull=False, then=F('classificacao__parent__codigo')),
                default=F('classificacao__codigo')
            )
        ).values('grupo_nome', 'grupo_codigo').annotate(
            total=Count('id')
        ).order_by('grupo_codigo')

        por_classificacao_especifica = queryset.filter(
        ...
        por_perito = peritos_queryset.annotate(
            total_ocorrencias=Coalesce(
                Count('ocorrencias_atribuidas', filter=Q(ocorrencias_atribuidas__in=queryset)), 0
            ),
            finalizadas=Coalesce(
                Count('ocorrencias_atribuidas', filter=Q(
            ...
            'nome_completo', 'total_ocorrencias', 'finalizadas', 'em_analise'
        ).order_by('-total_ocorrencias')

        servicos_queryset = ServicoPericial.objects.filter(deleted_at__isnull=True)
        por_servico = servicos_queryset.annotate(
            total=Coalesce(
                Count('ocorrencias', filter=Q(ocorrencias__in=queryset)), 0
            ),
            finalizadas=Coalesce(
                Count('ocorrencias', filter=Q(
                    ocorrencias__in=queryset,
                    ocorrencias__status='FINALIZADA'
                )), 0
            ),
            em_analise=Coalesce(
                Count('ocorrencias', filter=Q(
                    ocorrencias__in=queryset,
                    ocorrencias__status='EM_ANALISE'
                )), 0
            )
        ).values('sigla', 'nome', 'total', 'finalizadas', 'em_analise').order_by('-total')
