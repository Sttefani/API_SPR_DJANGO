...
class LaudoReferenciaAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Informações Básicas", {"fields": ("titulo", "tipo_exame", "arquivo_pdf")}),
        ("Processamento", {"fields": ("texto_extraido", "processado", "pasta_origem")}),
...
class TemplateLaudoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identificação", {"fields": ("tipo", "nome", "descricao", "ativo")}),
        (
            "Template",
            {
    ...
    def testar_preenchimento(self, request, queryset):
        ...
        for template in queryset:
            try:
                ...
                self.message_user(
                    request, f'✅ Template "{template.nome}" preenchido com sucesso!'
                )
            except Exception as e:
                self.message_user(
                    request,
                    f'❌ Erro no template "{template.nome}": {str(e)}',
                    level="ERROR",
                )

    testar_preenchimento.short_description = (
        "🧪 Testar preenchimento com dados de exemplo"
    )
...
class LaudoGeradoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            ...
            {"fields": ("dados_preenchimento",), "classes": ("collapse",)},
        ),
        (
            "Laudo Gerado",
            {
        ...
        ("Arquivos", {"fields": ("pdf_arquivo",), "classes": ("collapse",)}),
        ("Metadados", {"fields": ("gerado_por", "gerado_em")}),
    )
    ...
    def exportar_pdf(self, request, queryset):
        ...
        for laudo in queryset:
            if not laudo.pdf_arquivo:
                ...
                self.message_user(
                    request,
                    f'⚠️ Geração de PDF ainda não implementada para "{laudo}"',
                    level="WARNING",
                )
            else:
                count += 1
